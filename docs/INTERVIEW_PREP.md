...
**1. How do you guarantee there are no hand-typed numbers in your paper?**
The paper's tables are `\input` from files generated by `reproduce.sh` from the experiment DB, and figures come from the same outputs. Any table without data renders as a red PENDING box naming the exact run that fills it. `reproduce.sh` refuses to write paper tables from anything but the real DB, so smoke-test numbers can't leak in. CI regenerates the tables from the smoke DB (into a scratch directory) and compiles the paper on every push.

**2. Your headline is marked PENDING. Isn't that a weakness?**
It's the honest state: the heavy runs (GPU training of dozens of models, AC-power latency studies) haven't been executed, and writing a headline before seeing data is how people end up fitting the story to the hypothesis. The infrastructure is built so that when the runs land, one command regenerates every figure and table. What's claimable today is the systems and methodology contribution: the protocol, the exact kernel lowering, and the measurement pitfalls caught along the way.

**3. What's the single most important threat to the validity of the early-prediction result?**
That the predictor learns configuration identity or model size rather than anything about the signals. Three defenses:
- Leave-one-configuration-out evaluation, so seeds of a held-out configuration never appear in training.
- A params-only baseline the signal model must beat.
- Ablations naming which signal group carries the information.

A second threat is target leakage through the signals. They're computed on a fixed *training* batch, never test data.

**4. What would you do differently with 10× the compute?**
- ImageNet-scale validation of the ladder against the published AdaRound/BRECQ numbers, which is currently pending on data access.
- More seeds (≥10 processes and ≥5 seeds, so the percentile-bootstrap CIs stop under-covering).
- Multi-threaded kernels, so absolute latencies are competitive rather than just comparable.
- Real energy measurement.
- The small-LM track, to test whether compression erodes post-training behaviour before raw capability.

**5. Which part of this project are you proudest of, and which would you cut?**
Proudest: the measurement discipline. Fresh-process replication, fingerprints, exact per-layer engine-vs-simulation tests, and synthetic-DB tests for every analysis caught four bugs that would have silently corrupted published numbers:
- Linux memory counters inheriting the parent's peak
- a rounding-tie mismatch between the kernel and the simulation
- zero FLOPs reported for quantized models
