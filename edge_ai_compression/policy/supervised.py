...
def suggest_config_from_table(csv_path: str | Path, constraints: dict[str, float]) -> dict[str, Any]:
    ...
    sub = df[(df["latency_mean"] <= max_lat) & (df["size_mb"] <= max_size) & (df["accuracy"] >= min_acc)]
    if sub.empty:
        sub = df
